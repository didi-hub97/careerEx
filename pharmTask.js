const drugs = [
  ...
  drugs.forEach(drug => console.log(`Drug: ${drug.name}, Manufacturer: ${drug.manufacturer}`));
  ...
  const drugDosages = drugs.map(drug => `Drug: ${drug.name} - ${drug.dosageMg}mg`);
  console.log(drugDosages);
